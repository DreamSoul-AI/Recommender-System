...
from .layers import AveragePooling
...
class SASRec(nn.Module):
    ...
    def user_embedding(self, user, item_hist):
        ...
        for i in range(len(self.attention_layers)):
            ...
            item_hist_embedding *= ~mask.unsqueeze(-1)
        item_hist_embedding = self.last_layernorm(item_hist_embedding)
        item_hist_embedding = self.pooling_layer(item_hist_embedding, mask)
        user_embedding = self.user_weight(user)
        user_embedding = user_embedding + item_hist_embedding
