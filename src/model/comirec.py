...
from .layers import MultiInterestSA


# https://github.com/datawhalechina/torch-rechub/blob/main/torch_rechub/models/matching/comirec.py

class ComiRec(nn.Module):
    def __init__(self, num_users, num_items, embedding_mode, max_length, hidden_size, interest_num):
        ...
        self.multi_interest_sa = MultiInterestSA(embedding_dim=self.hidden_size,
    ...
    def user_embedding(self, user, item_hist):
        ...
        multi_interest_emb = self.multi_interest_sa(item_hist_embedding, mask.unsqueeze(-1))  # [256, 4, 16]
...
def comirec(cfg):
    ...
    model = ComiRec(num_users, num_items, embedding_mode, max_length, **cfg['comirec'])
