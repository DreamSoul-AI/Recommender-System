...
class FeatureEncoder(object):
    ...
    def preprocess(self, ddf):
        ...
        """In summary, this method selectively preprocesses columns of a dataframe by filling in null values, 
        applying specified preprocessing functions, changing data types, and finally keeping only the active columns. 
        The specifics of how each column is processed depend on the configuration in self.feature_cols, self.label_col, 
        and self.dtype_dict."""
    ...
    def fit_categorical_col(self, feature_col, data_vector, min_categr_count=1, num_buckets=10):
        ...
        #pdb.set_trace()
#         self.feature_map.feature_specs
# OrderedDict([('query_index', {'source': '', 'type': 'index'}), 
#              ('corpus_index', {'source': '', 'type': 'index'}), 
#              ('user_id', {'source': 'user', 'type': 'categorical', 'min_categr_count': 1, 
#                           'vocab_size': 7069})])
